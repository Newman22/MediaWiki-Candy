...
class CandyTemplate extends BaseTemplate {
	...
	public function execute() {
		...
		$html .= Html::rawElement( 'div', [ 'id' => 'content', 'class' => 'mw-body',  'role' => 'main' ],
			...
			Html::rawElement( 'div', [ 'id' => 'bodyContent', 'class' => 'mw-body-content' ],
				...
				Html::element( 'a', [ 'href' => '#column-one', 'class' => 'mw-jump-link' ],
					$this->getMsg( 'candy-jumptonavigation' )->text()
				) .
				Html::element( 'a', [ 'href' => '#searchInput', 'class' => 'mw-jump-link' ],
					$this->getMsg( 'candy-jumptosearch' )->text()
				) .
				'<!-- start content -->' .
		...
		$html .= Html::rawElement( 'div',
			...
			$this->getMobileNavigationIcon(
				'p-personal',
				$this->getMsg( 'candy-jumptopersonal' )->text()
			) .
			$this->getMobileNavigationIcon(
				'globalWrapper',
				$this->getMsg( 'candy-jumptotop' )->text()
			)
		);
	...
	protected function getCactions() {
		...
		// Normal cactions
		if ( $tabCount > 2 ) {
			$html .= $this->getBox( 'cactions', $allTabs, 'candy-cactions-label' );
		} else {
		...
		if ( $tabCount !== 1 ) {
			...
			$tabs['more'] = [
				'text' => $this->getMsg( 'candy-more-actions' )->text(),
				'href' => '#p-cactions',
				'id' => 'ca-more'
			];
		}
		...
		$html .= $this->getBox( 'cactions-mobile', $tabs, 'candy-cactions-label' );
